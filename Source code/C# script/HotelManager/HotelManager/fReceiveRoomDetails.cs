...
namespace HotelManager
{
    public partial class fReceiveRoomDetails : Form
    {
        ...
        public void ShowReceiveRoom(int idReceiveRoom)
        {
            ...
            txbIDReceiveRoom.Text = ((int)data["Check-In ID"]).ToString();
            txbRoomName.Text = data["Room Name"].ToString();
            txbDateCheckIn.Text = ((DateTime)data["Check-In Date"]).ToString().Split(' ')[0];
            txbDateCheckOut.Text= ((DateTime)data["Check-Out Date"]).ToString().Split(' ')[0];
        }
        ...
        private void bunifuThinButton22_Click(object sender, EventArgs e)
        {
            ...
            if (idCustomer != CustomerDAO.Instance.GetIDCustomerFromBookRoom(idReceiveRoom))
            {
                ...
                MessageBox.Show("Customer removed successfully!", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
                ShowCustomers(idReceiveRoom);
            }
            else
